...
def map_and_filter(s, map_fn, filter_fn):
    ...
    # BEGIN Question 0
    "*** REPLACE THIS LINE ***"
    return [map_fn(element) for element in s if filter_fn(map_fn(element))is True]
    # END Question 0
run_docstring_examples(map_and_filter,globals(),True)
def key_of_min_value(d):
    ...
    # BEGIN Question 0
    "*** REPLACE THIS LINE ***"
    return list.pop([key for key in dict.keys(d) if d[key]==(min(dict.values(d)))])
    # END Question 0
...
def enumerate(s, start=0):
    ...
    """
    # BEGIN Question 0
    return [[start+s.index(element),element] for element in s ]
    "*** REPLACE THIS LINE ***"
    # END Question 0
