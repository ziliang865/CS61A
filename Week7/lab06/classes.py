...
class Player(object):
    ...
    def take(self, thing):
        ...
        if thing not in self.place.things:
            ...
            thing_obj=self.place.things[thing]
            self.backpack.append(thing_obj)
            print("Player takes the",thing)
            dict.pop(self.place.things,thing)
            return
        "*** YOUR CODE HERE ***"
    ...
    def knapsack(self, max_weight, list_of_treasures):
        ...
        """
        max_value_dict={}
        def knapsack_helper(max_weight,list_of_treasures):
            nonlocal max_value_dict
            treasure_tuple=tuple(list_of_treasures)
            if (treasure_tuple,max_weight) in max_value_dict:
                 return max_value_dict[treasure_tuple]
            if len(list_of_treasures)==0:
                return 0
            else:
                    max_without_first=knapsack_helper(max_weight,list_of_treasures[1:])
                    if list_of_treasures[0].weight > max_weight:
                        max_value_dict[(treasure_tuple,max_weight)] = max_without_first
                        return max_without_first
                    else:
                        max_with_first = list_of_treasures[0].value + knapsack_helper(\
                            max_weight - list_of_treasures[0].weight, list_of_treasures[1:])
                        if max_with_first>max_without_first:
                            max_value_dict[(treasure_tuple,max_weight)]=max_with_first
                            return max_with_first
                        else:
                            max_value_dict[(treasure_tuple,max_weight)]=max_without_first
                            return max_without_first
        return knapsack_helper(max_weight,list_of_treasures)
...
class Key(Thing):
    def use(self,place_obj):
        if place_obj.locked==True:
            place_obj.locked=False
            print(place_obj.name,"is now unlocked!")
